// Config File for << JOYSTICK SPACEMOUSE >>
...
/* The user specific settings, like pin mappings or special configuration variables and
   sensitivities are stored in config.h. This file is meant for the << JOYSTICK SPACEMOUSE >>
   >>> Some comments distinguish between resistive-JOYSTICKs "RJS" and hall-JOYSTICKs "HJS" to give
   examples for values. <<< Please adjust your settings and save it as --> config.h <-- !
*/
...
(This definition of X and Y may not correspond to the print on your joysticks... We will find out
which signal is X and Y now.)
...
3. At the joystick in front of you (A), move the joystick from the top -> down to bottom (X) and
observe the debug output: a) AX goes from 0 to 1023 -> Everything is correct. b) AX goes from 1023
to 0 -> You need to invert AX, see INVERTLIST below. c) Another output is showing movement: Swap the
pins in the PINLIST. Probably you have to swap the first and second element, as AX and AY may be
swapped.

If you have the joystick TeachingTech recommended:
  The pins labelled X and Y on the joystick are NOT the X and Y needed here, but swapped. First
joysticks Y: AX and X: AY.

4. Continue with AY: Move the joystick from left -> right and the values shall increase from 0 to
1023 and follow the instructions 3.a) to 3.c) above.
5. Repeat this with every axis and every joystick until you have a valid PINLIST and maybe an
INVERTLIST
6. If you use hall-effect-joysticks, you should observe the value shown when the joystick is not
touched. At this zeropoint it should read roundabout 512. If it doesn't, you can mechanically adjust
the joysticks - there are many tutorial videos online that show the procedure.
*/
...
should be idle or the mouse is too sensitive to subtle movements. Semi-automatic: Set debug = 11.
Don't touch the mouse and observe the automatic output. Manual:         Set debug = 2.  Don't touch
the mouse but observe the values. They should be nearly to zero. Every value around zero which is
noise or should be neglected afterwards is in the following deadzone.
*/
...
3. (a) Start out with AX (positive values)
   (b) Start moving the your Spacemouse and try increasing the value of AX till you can't get a
higher value out of it. (c) this is your positive maximum value for AX so write it down for AX
...
for and hit enter. You will see Values TX, TY, TZ, RX, RY, RZ
...
#define GATE_NTZ                                                                                   \
  0.01 // RJS:0.01  HJS:8.00  // gate value, which negative z movements will be ignored (like an
...
When pushing or pulling, the knob produced transient rotational components that stops when the
z-translation gets the priority. So when pulling, we get first a rotation then the desired
translation. So this mode sees that min. 3 of 4 joysticks all move up (or down) and use it as an
indicator that the knob is mainly pushed/pulled. So before any (ghost-)rotational component can be
calculated, it is sorted out.
*/
