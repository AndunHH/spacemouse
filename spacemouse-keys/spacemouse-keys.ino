...
// One good starting point is the work and video by TeachingTech:
// https://www.printables.com/de/model/864950-open-source-spacemouse-space-mushroom-remix Then
// follow along on github, how we reached this state of the source code.
...
// setup the keys e.g. to internal pull-ups
  ...
  // Serial.begin(115200); // irrelevant for Arduino Micro-platform because the baudrate is set by
  // the connected PC
 ...
 */
void loop() {
  // the debug mode can be set during runtime via the serial interface. See config.h for a
  // description of the different debug modes.
  ...
  if ((debug != 20) &&
      (debug !=
       30)) { // SNo: don't change debug-mode/menu when calcMinMax() or parameterMenu() are running
    double num;
...
// Report back 0-1023 raw ADC 10-bit values if enabled
  ...
  if ((par.values->compEnabled == 1) &&
      (debug != 20)) { // only when not in debug 20 = find min/max values
  ...
  // Report centered joystick values if enabled. Values should be approx -350 to +350, jitter around
  // 0 at idle
  if (debug == 2) {
  ...
  // Report centered joystick values. Filtered for deadzone. Approx -350 to +350, locked to zero at
  // idle
  if (debug == 3) {
    debugOutput2(centered);
  }
...
// if the kill-key feature is enabled, rotations or translations are killed=set to zero
  ...
  // exclusive mode: rotation OR translation, but never both at the same time to avoid issues with
  // classics joysticks
...
#ifdef LEDpin
/// @brief Turn on or off a simple led. The pin is defined by LEDpin in config.h. If the LED needs
/// to be inverted, define LEDinvert in config.h
 ...
 */
void setAnalogReferenceVoltage(int dbg) {
  if (dbg == 1) { // Set the reference voltage for the AD Convertor to 5V only for the first
                  // calibration step (pinout/inversion calibration).
    analogReference(DEFAULT);
#ifdef DEBUG_ADC
    Serial.println(F("Setting analog reference to 5V."));
#endif
  } else { // Set the reference voltage for the AD Convertor to 2.56V in order to get larger
           // sensitivity.
  ...
  // The first measurements after changing the reference voltage can be wrong. So take 100ms to let
  // the voltage stabilize and take some measurements afterwards just to be sure. Performancewise
  // this isn't a problem due to the debug/setup nature of this function.
  delay(100);
  int tempReads[8];
  for (int i = 0; i <= 8; i++) {
